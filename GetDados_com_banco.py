...
def update_data(n_intervals, connection_data):
    global piranometer_client, cr1000_client, historical_data, conn
    ...
    try:
        ...
        try:
            ...
            print(f"Erro ao gravar no banco: {db_error}")

        piranometer_table = [
            {"parameter": "Irradiância solar", "value": f"{piranometer_data.get('irradiance', float('nan')):.2f}", "unit": "W/m²"},
            {"parameter": "Tensão de saída", "value": f"{piranometer_data.get('voltage_out', float('nan')):.4f}", "unit": "mV"},
        ...
        fig = {
            'data': [
                {'x': historical_data['timestamp'], 'y': historical_data['irradiance'], 'type': 'line', 'name': 'Irradiância (W/m²)', 'yaxis': 'y1'},
                {'x': historical_data['timestamp'], 'y': historical_data['cr1000_1'], 'type': 'line', 'name': 'Canal 1', 'yaxis': 'y2', 'line': {'color': 'red'}},
                {'x': historical_data['timestamp'], 'y': historical_data['cr1000_2'], 'type': 'line', 'name': 'Canal 2', 'yaxis': 'y2', 'line': {'color': 'green'}},
                {'x': historical_data['timestamp'], 'y': historical_data['cr1000_3'], 'type': 'line', 'name': 'Canal 3', 'yaxis': 'y2', 'line': {'color': 'purple'}}
            ],
            'layout': {
                'title': 'Irradiância e Valores dos Canais CR1000',
                'xaxis': {'title': 'Tempo'},
                'yaxis': {'title': 'W/m²', 'side': 'left', 'color': 'blue'},
                'yaxis2': {'title': 'Valores CR1000', 'side': 'right', 'overlaying': 'y', 'color': 'red'},
                'uirevision': 'constant'
            }
        }
        ...
        print(f"Erro no callback de aquisição: {e}"),
        raise PreventUpdate
